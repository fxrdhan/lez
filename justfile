...
[group('house-keeping')]
genDemo:
    fish_prompt="> " fish_history="lsr_history" vhs < docs/tapes/demo.tape
...
# compile the lsr binary
[group('building')]
@build:
    cargo build

# compile the lsr binary (in release mode)
[group('building')]
@build-release:
...
# check that the lsr binary can compile
[group('building')]
@check:
    cargo check
...
release:
    ...
    git commit -asm "chore: lsr v{{new_version}} changelogs, version bump"
    git push
...
@gh-release:
    ...
    git tag --sign -a "v{{new_version}}" -m "auto generated by the justfile for lsr v$(convco version)"
    just cross
    just mangen
    just completions
    ...
    gh release create "v$(convco version)" --target "$(git rev-parse HEAD)" --title "lsr v$(convco version)" -d -F ./target/"release-notes-$(convco version)/RELEASE.md" ./target/"bin-$(convco version)"/* ./target/"man-$(convco version).tar.gz" ./target/"completions-$(convco version).tar.gz"
...
# Generate release binaries for lsr
#
# usage: cross
[group('binaries')]
@cross:
    ...
    ## Linux
    ### x86
    just binary lsr x86_64-unknown-linux-gnu
    # just binary_static lsr x86_64-unknown-linux-gnu
    just binary lsr x86_64-unknown-linux-musl
    # just binary_static lsr x86_64-unknown-linux-musl

    ### aarch
    just binary lsr aarch64-unknown-linux-gnu
    just binary_no_libgit lsr aarch64-unknown-linux-gnu
    # BUG: just binary_static lsr aarch64-unknown-linux-gnu

    ### arm
    just binary lsr arm-unknown-linux-gnueabihf
    just binary_no_libgit lsr arm-unknown-linux-gnueabihf
    # just binary_static lsr arm-unknown-linux-gnueabihf

    ## MacOS
    # TODO: just binary lsr x86_64-apple-darwin

    ## Windows
    ### x86
    just binary lsr.exe x86_64-pc-windows-gnu
    # just binary_static lsr.exe x86_64-pc-windows-gnu
    # TODO: just binary lsr.exe x86_64-pc-windows-gnullvm
    # TODO: just binary lsr.exe x86_64-pc-windows-msvc
