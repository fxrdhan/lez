...
pub fn get_command() -> clap::Command {
    clap::Command::new(clap::crate_name!())
        ...
        .arg(arg!(--"no-system" "do not show system files (Windows only)")
            .alias("hide-system")
            .alias("no-system-files"))
        .arg(arg!(--"no-hidden-attrib" "do not show entries with the Windows hidden attribute (Windows only)")
            .alias("hide-hidden-attrib")
            .alias("no-hidden-attributes"))
        .arg(arg!(--"no-hidden-links" "do not show Windows hidden profile links / junctions (Windows only)")
            .alias("no-junctions"))
        ...
        .arg(arg!(--"percent-digits" <NUM> "number of decimal digits to display for percentages (0..=8, default: 1)")
            .alias("precision-percent")
