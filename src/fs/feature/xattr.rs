...
/// Never happens on macOS, but happens on Linux with files and folders copied from macOS.
#[cfg(target_os = "macos")]
...
pub fn display_tags(attribute: &Attribute) -> Option<Vec<Tag>> {
    if check_tags_name(&attribute.name) {
        attribute
            .value
            .as_ref()
...
/// See its [`std::str::FromStr`] implementation for supported mappings.
///
