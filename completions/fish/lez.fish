...
complete -c lez -l no-system -d "Do not show system files (Windows only)"
complete -c lez -l no-hidden-attrib -d "Do not show entries with the Windows hidden attribute (Windows only)"
complete -c lez -l no-hidden-links -d "Do not show Windows hidden profile links / junctions (Windows only)"
...
complete -c lez -s s -l sort -d "Which field to sort by" -x -a "
    ...
    code\t'Sort by lines of code'
    filesize\t'Sort by file size'
    lines\t'Sort by lines of code'
    loc\t'Sort by lines of code'
    percent\t'Sort by code percentage'
    percentage\t'Sort by code percentage'
...
complete -c lez -s S -l blocksize -d "List the allocated size of each file, in bytes"
complete -c lez -l blocks -d "List the allocated size of each file, in blocks"
