...
complete -c eza -l no-system -d "Do not show system files (Windows only)"
complete -c eza -l no-hidden-attrib -d "Do not show entries with the Windows hidden attribute (Windows only)"
complete -c eza -l no-hidden-links -d "Do not show Windows hidden profile links / junctions (Windows only)"
